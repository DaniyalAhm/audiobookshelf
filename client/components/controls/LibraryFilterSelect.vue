<template>
  <div ref="wrapper" class="relative" v-click-outside="clickOutside">
    <div class="relative h-7">
      <button type="button" class="select-trigger-btn bg-bg sm:text-sm" aria-haspopup="menu" :aria-expanded="showMenu" @click.prevent="showMenu = !showMenu">
        <span class="flex items-center justify-between">
          <span class="block truncate text-xs" :class="!selectedText ? 'text-secondary-text' : ''">{{ selectedText }}</span>
        </span>
      </button>
      <span v-if="selected === 'all'" class="ml-3 absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
        <svg class="h-5 w-5 text-secondary-text" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
          <path fill-rule="evenodd" d="M10 3a1 1 0 01.707.293l3 3a1 1 0 01-1.414 1.414L10 5.414 7.707 7.707a1 1 0 01-1.414-1.414l3-3A1 1 0 0110 3zm-3.707 9.293a1 1 0 011.414 0L10 14.586l2.293-2.293a1 1 0 011.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z" clip-rule="evenodd" />
        </svg>
      </span>
      <button v-else :aria-label="$strings.ButtonClearFilter" class="ml-3 absolute inset-y-0 right-0 flex items-center pr-2 cursor-pointer text-secondary-text hover:text-black-500" @mousedown.stop @mouseup.stop @click.stop.prevent="clearSelected">
    ...
    <div v-show="showMenu" class="absolute z-10 mt-1 w-full bg-bg border border-black-200 shadow-lg rounded-md py-1 ring-1 ring-black/5 overflow-auto focus:outline-hidden text-sm libraryFilterMenu">
      <ul v-show="!sublist" class="h-full w-full" role="menu">
        <template v-for="item in selectItems">
          <li :key="item.value" class="select-none relative py-2 pr-9 cursor-pointer hover:bg-white/5" :class="item.value === selected ? 'bg-white/5 text-primary' : 'text-black-500 hover:text-white'" role="menuitem" :aria-haspopup="item.sublist ? '' : 'menu'" @click="clickedOption(item)">
            ...
            <div v-if="item.value === selected" class="absolute inset-y-0 right-2 h-full flex items-center pointer-events-none">
              <span class="material-symbols text-base text-primary">check</span>
            </div>
          </li>
        </template>
      </ul>
      <ul v-show="sublist" class="h-full w-full" role="menu">
        <li class="text-black-500 select-none relative py-2 pl-9 cursor-pointer hover:bg-white/5" role="menuitem" @click="sublist = null">
        ...
        <li v-if="!sublistItems.length" class="text-secondary-text select-none relative px-2" role="menuitem">
        ...
        <template v-for="item in sublistItems">
          <li :key="item.value" class="select-none relative px-2 cursor-pointer hover:bg-white/5" :class="`${sublist}.${item.value}` === selected ? 'bg-white/5 text-primary' : 'text-black-500 hover:text-white'" role="menuitem" @click="clickedSublistOption(item.value)">
