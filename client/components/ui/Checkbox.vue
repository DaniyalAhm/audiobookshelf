<template>
  <label tabindex="0" ref="labelRef" class="flex justify-start items-center" :class="!disabled ? 'cursor-pointer' : ''" @keydown.enter="enterKeydown">
    <div class="border-2 rounded-sm flex shrink-0 justify-center items-center" :class="wrapperClass">
      ...
      <span v-if="partial" class="material-symbols text-base leading-none text-secondary-text">remove</span>
    ...
    <div v-if="label" class="select-none" :class="[labelClassname, disabled ? 'text-secondary-text' : 'text-black-500']">{{ label }}</div>
  </label>
</template>
