<template>
  <div class="w-full">
    <label v-if="label" class="px-1 text-sm font-semibold" :class="disabled ? 'text-secondary-text' : ''">{{ label }}</label>
    <div ref="wrapper" class="relative">
      <form @submit.prevent="submitForm">
        <div ref="inputWrapper" class="input-wrapper flex-wrap relative w-full flex items-center border border-black-200 rounded-lg px-2 py-2" :class="disabled ? 'pointer-events-none bg-black-50 text-muted' : 'bg-fg text-black-500'">
      ...
      <ul ref="menu" v-show="isFocused && itemsToShow.length" class="absolute z-50 mt-0 w-full bg-fg border border-black-200 shadow-lg max-h-56 rounded-lg py-1 text-base ring-1 ring-black/5 overflow-auto focus:outline-hidden sm:text-sm" role="listbox" aria-labelledby="listbox-label">
        <template v-for="item in itemsToShow">
          <li :key="item" class="text-black-500 select-none relative py-2 pr-3 cursor-pointer hover:bg-black-400" :class="isMenuItemSelected(item) ? 'text-primary' : ''" role="option" @click="clickedOption($event, item)" @mouseup.stop.prevent @mousedown.prevent>
            <div class="flex items-center">
              <span class="font-normal ml-3 block truncate">{{ item }}</span>
            </div>
            <span v-if="input === item" class="text-primary absolute inset-y-0 right-0 flex items-center pr-4">
