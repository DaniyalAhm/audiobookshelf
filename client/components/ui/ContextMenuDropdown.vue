<template>
  <div class="relative h-9 w-9" v-click-outside="clickOutsideObj">
    <slot :disabled="disabled" :showMenu="showMenu" :clickShowMenu="clickShowMenu" :processing="processing">
      <button v-if="!processing" type="button" :disabled="disabled" class="relative h-full w-full flex items-center justify-center pl-3 pr-3 text-left focus:outline-hidden cursor-pointer text-black-500 hover:text-black-500 rounded-full hover:bg-white/5" :aria-label="$strings.LabelMore" aria-haspopup="menu" :aria-expanded="showMenu" @click.stop.prevent="clickShowMenu">
    ...
    <transition name="menu">
      <div v-show="showMenu" ref="menuWrapper" role="menu" class="absolute right-0 mt-2 z-10 bg-bg border border-black-200 shadow-lg rounded-xl py-1 focus:outline-hidden sm:text-sm animated-menu" :style="{ width: menuWidth + 'px' }">
        <template v-for="(item, index) in items">
          <template v-if="item.subitems">
            ...
            <div
              ...
              class="absolute bg-bg border rounded-xl border-black-200 shadow-lg z-50 -ml-px py-1 animated-menu"
